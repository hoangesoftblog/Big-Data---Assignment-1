...
curl -s \
  ...
  "config": {
    ...
    "contactPoints": "cassandradb",
    "loadBalancing.localDc": "datacenter1",
    "topic.faker.kafkapipeline.fakerdata.mapping": "name=value.name, address=value.address, year=value.year, username=value.username, phone=value.phone, workplace=value.workplace, email=value.email, birthdate=value.birthdate, isbn10=value.isbn10, favorite_color=value.favorite_color",
    "topic.faker.kafkapipeline.fakerdata.consistencyLevel": "LOCAL_QUORUM"
  }
}'

echo "Starting Bitcoin Sink"
curl -s \
     -X POST http://localhost:8083/connectors \
     -H "Content-Type: application/json" \
     -d '{
  ...
  "config": {
    ...
    "contactPoints": "cassandradb",
    "loadBalancing.localDc": "datacenter1",
    "topic.bitcoin.kafkapipeline.bitcoinprice.mapping": "time=value.time, price=value.price",
    "topic.bitcoin.kafkapipeline.bitcoinprice.consistencyLevel": "LOCAL_QUORUM"
  }
}'
echo "Done."